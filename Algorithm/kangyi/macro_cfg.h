#ifndef _KANGYI_MACRO_CFG_H_
#define _KANGYI_MACRO_CFG_H_
...
#define ONE_LINE_BYTE_NUM		24			//曾意给cpu一排数据的字节数
...
{
	DISTANCE_MM_IR_X = 60,				//71.6mm			1.18682387
	OFFSET_BM_IR_Y = 15,					// 大磁 和同一排上的计数红外的Y轴偏移码盘数

	OFFSET_LMM_IR_X = -21,				// 左中磁 与同一排上的计数红外的X轴上的偏移码盘
	OFFSET_RMM_IR_X = 21,				// 右中磁 与同一排上的计数红外的X轴上的偏移码盘
	OFFSET_MM_IR_Y = 5,					// 中磁 和同一排上的计数红外的Y轴偏移码盘数，单位:个数
	MM_WHEEL_ERR = 3,					// 中磁 码盘允许误差

	OFFSET_LM1_IR_X = -50,				// 左边磁 与对应红外再X轴上的偏移码盘数，单位：个数
	OFFSET_RM1_IR_X = 50,				// 右边磁 与对应红外再X轴上的偏移码盘数，单位：个数
	OFFSET_LRM_IR_Y = 1,				// 边磁 和同一排上的计数红外的Y轴偏移码盘数，单位:个数
	SM_WHEEL_ERR = 8,					// 边磁 码盘允许误差

	Gap_MAG = 8,						//各个边磁之间的中心间距，单位：码盘个数
