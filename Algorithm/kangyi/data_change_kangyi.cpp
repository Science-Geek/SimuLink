...
void MMSM_DataChange_KANGYI(OneDSignal *TdnData_p, TdnData_Alg *AlgData_p)
{
	int i = 0;
	int MM_Flag = 0;
	...
	for (i = 0; i < SM_NUM; i++)
	{
		MMSM_DataChangeOne_TMR(&TdnData_p[TDN_LM1 + i], &AlgData_p->LSM[SM_NUM - 1 - i], MM_PrepStartWheel, MM_PrepEndwheel, Slope_MM, i , MM_Flag, OFFSET_LM1_IR_X, OFFSET_LRM_IR_Y, SM_WHEEL_ERR);
		MMSM_DataChangeOne_TMR(&TdnData_p[TDN_RM1 + i], &AlgData_p->RSM[SM_NUM - 1 - i], MM_PrepStartWheel, MM_PrepEndwheel, Slope_MM, i, MM_Flag, OFFSET_RM1_IR_X, OFFSET_LRM_IR_Y, SM_WHEEL_ERR);
...
void DataChange(int LastError)
{
	...
	MMSM_DataChange_KANGYI(AlgSignal, AlgDataBuf);
