#ifndef _JULONG_TMR_MACRO_CFG_H_
#define _JULONG_TMR_MACRO_CFG_H_
...
{
	DISTANCE_MM_IR_X = 65,				//72.5mm		码盘单距	1.115
	OFFSET_BM_IR_Y = 5,					// 大磁 和同一排上的计数红外的Y轴偏移码盘数

	OFFSET_LMM_IR_X = -24,				// 左中磁 与同一排上的计数红外的X轴上的偏移码盘
	OFFSET_RMM_IR_X = 24,				// 右中磁 与同一排上的计数红外的X轴上的偏移码盘
	OFFSET_MM_IR_Y = 6,					// 中磁 和同一排上的计数红外的Y轴偏移码盘数，单位:个数
	MM_WHEEL_ERR = 11,					// 中磁 码盘允许误差

	OFFSET_LM1_IR_X = -28,				// 左边磁 与对应红外再X轴上的偏移码盘数，单位：个数
	OFFSET_RM1_IR_X = 28,				// 右边磁 与对应红外再X轴上的偏移码盘数，单位：个数
	OFFSET_LRM_IR_Y = 15,				// 边磁 和同一排上的计数红外的Y轴偏移码盘数，单位:个数
	SM_WHEEL_ERR = 2,					// 边磁 码盘允许误差

	Gap_MAG = 7,						//各个边磁之间的中心间距，单位：码盘个数
